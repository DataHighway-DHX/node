...
pub trait Trait: system::Trait + roaming_operators::Trait + mining_speed_boosts_configuration_token_mining::Trait {
    ...
    type MiningSpeedBoostClaimsTokenMiningClaimDateRedeemed: Parameter + Member + SimpleArithmetic + Bounded + Default + Copy;
  }
...
decl_storage! {
    trait Store for Module<T: Trait> as MiningSpeedBoostClaimsTokenMining {
        ...
        pub MiningSpeedBoostClaimsTokenMinings get(fn mining_speed_boosts_claims_token_mining): map T::MiningSpeedBoostClaimsTokenMiningIndex => Option<MiningSpeedBoostClaimsTokenMining>;
        ...
        pub MiningSpeedBoostClaimsTokenMiningOwners get(fn mining_speed_boosts_claims_token_mining_owner): map T::MiningSpeedBoostClaimsTokenMiningIndex => Option<T::AccountId>;

        /// Stores mining_speed_boosts_claims_token_mining_claims_result
        pub MiningSpeedBoostClaimsTokenMiningClaimResults get(fn mining_speed_boosts_claims_token_mining_claims_results): map (T::MiningSpeedBoostConfigurationTokenMiningIndex, T::MiningSpeedBoostClaimsTokenMiningIndex) =>
        ...
        pub TokenMiningClaimConfiguration get(fn token_mining_claim_configuration): map T::MiningSpeedBoostClaimsTokenMiningIndex => Option<T::MiningSpeedBoostConfigurationTokenMiningIndex>;

        /// Get mining_speed_boosts_claims_token_mining_id's belonging to a mining_speed_boosts_configuration_token_mining_id
        pub TokenMiningConfigurationClaims get(fn token_mining_configuration_claims): map T::MiningSpeedBoostConfigurationTokenMiningIndex => Option<Vec<T::MiningSpeedBoostClaimsTokenMiningIndex>>
    }
}
...
impl<T: Trait> Module<T> {
    ...
    ) -> Result<(), &'static str>
    {
        ...
        if let Some(configuration_claims) = Self::token_mining_configuration_claims(mining_speed_boosts_configuration_token_mining_id) {
            debug::info!("Configuration id key {:?} exists with value {:?}", mining_speed_boosts_configuration_token_mining_id, configuration_claims);
            let not_configuration_contains_claim = !configuration_claims.contains(&mining_speed_boosts_claims_token_mining_id);
            ...
            debug::info!("Associated claim {:?} with configuration {:?}", mining_speed_boosts_claims_token_mining_id, mining_speed_boosts_configuration_token_mining_id);
            Ok(())
        } else {
            debug::info!("Configuration id key does not yet exist. Creating the configuration key {:?} and appending the claim id {:?} to its vector value", mining_speed_boosts_configuration_token_mining_id, mining_speed_boosts_claims_token_mining_id);
            <TokenMiningConfigurationClaims<T>>::insert(mining_speed_boosts_configuration_token_mining_id, &vec![mining_speed_boosts_claims_token_mining_id]);
            Ok(())
        }
    }
    ...
    fn insert_mining_speed_boosts_claims_token_mining(owner: &T::AccountId, mining_speed_boosts_claims_token_mining_id: T::MiningSpeedBoostClaimsTokenMiningIndex, mining_speed_boosts_claims_token_mining: MiningSpeedBoostClaimsTokenMining) {
        // Create and store mining mining_speed_boosts_claims_token_mining
        <MiningSpeedBoostClaimsTokenMinings<T>>::insert(mining_speed_boosts_claims_token_mining_id, mining_speed_boosts_claims_token_mining);
    ...
    fn update_owner(to: &T::AccountId, mining_speed_boosts_claims_token_mining_id: T::MiningSpeedBoostClaimsTokenMiningIndex) {
...
mod tests {
    ...
    impl mining_speed_boosts_configuration_token_mining::Trait for Test {
      ...
      type MiningSpeedBoostConfigurationTokenMiningTokenSecure = bool;
      ...
      type MiningSpeedBoostConfigurationTokenMiningTokenID = u64;
