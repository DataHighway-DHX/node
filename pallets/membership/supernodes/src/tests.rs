...
fn cant_add_duplicate_members() {
    ExternalityBuilder::build().execute_with(|| {
        ...
        assert_noop!(VecSet::add_member(Origin::signed(1)), Error::<TestRuntime>::AlreadyMember);
    })
}
...
fn cant_exceed_max_members() {
    ExternalityBuilder::build().execute_with(|| {
        ...
        assert_noop!(VecSet::add_member(Origin::signed(16)), Error::<TestRuntime>::MembershipLimitReached);
    })
}
...
fn remove_member_handles_errors() {
    ExternalityBuilder::build().execute_with(|| {
        ...
        assert_noop!(VecSet::remove_member(Origin::signed(2)), Error::<TestRuntime>::NotMember);
    })
}
