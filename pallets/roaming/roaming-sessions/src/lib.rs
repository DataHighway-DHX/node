...
decl_storage! {
    trait Store for Module<T: Trait> as RoamingSessions {
        ...
        pub RoamingSessions get(fn roaming_session): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<RoamingSession>;
        ...
        pub RoamingSessionOwners get(fn roaming_session_owner): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<T::AccountId>;

        /// Get roaming session join requests
        pub RoamingSessionJoinRequests get(fn roaming_session_join_requests): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<RoamingSessionJoinRequest<T::RoamingNetworkServerIndex, T::RoamingSessionJoinRequestRequestedAt>>;

        /// Get roaming session join accepts
        pub RoamingSessionJoinAccepts get(fn roaming_session_join_accepts): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<RoamingSessionJoinAccept<T::RoamingSessionJoinRequestAcceptExpiry, T::RoamingSessionJoinRequestAcceptAcceptedAt>>;

        /// Get roaming session device
        pub RoamingSessionDevices get(fn roaming_session_device): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<T::RoamingDeviceIndex>;

        /// Get roaming device sessions
        pub RoamingDeviceSessions get(fn roaming_device_sessions): map hasher(blake2_128_concat) T::RoamingDeviceIndex => Option<Vec<T::RoamingSessionIndex>>
    }
}
