...
decl_storage! {
    trait Store for Module<T: Trait> as RoamingPacketBundles {
        ...
        pub RoamingPacketBundles get(fn roaming_packet_bundle): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<RoamingPacketBundle>;
        ...
        pub RoamingPacketBundleOwners get(fn roaming_packet_bundle_owner): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<T::AccountId>;

        /// Get roaming packet_bundle price. None means not for sale.
        pub RoamingPacketBundlePrices get(fn roaming_packet_bundle_price): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<BalanceOf<T>>;

        // /// Get roaming packet_bundle receiver
        // pub RoamingPacketBundleReceivers get(fn roaming_packet_bundle_receivers): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<RoamingPacketBundleReceiver<T::RoamingPacketBundleNextBillingAt, T::RoamingPacketBundleFrequencyInDays>>;

        /// Get roaming packet_bundle receiver
        pub RoamingPacketBundleReceivers get(fn roaming_packet_bundle_receivers): map hasher(blake2_128_concat) (T::RoamingPacketBundleIndex, T::RoamingNetworkServerIndex) =>
        ...
        pub RoamingNetworkServerPacketBundles get(fn roaming_network_server_packet_bundles): map hasher(blake2_128_concat) T::RoamingNetworkServerIndex => Option<Vec<T::RoamingPacketBundleIndex>>;

        // Device Session mapping
        pub RoamingPacketBundleDeviceSession get(fn roaming_packet_bundle_device_sessions): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<(T::RoamingDeviceIndex, T::RoamingSessionIndex)>;

        pub RoamingDeviceSessionPacketBundles get(fn roaming_device_session_packet_bundles): map hasher(blake2_128_concat) (T::RoamingDeviceIndex, T::RoamingSessionIndex) => Option<Vec<T::RoamingPacketBundleIndex>>;

        // IPFS
        pub RoamingExternalDataStorageHashPacketBundle get(fn roaming_external_data_storage_hash_packet_bundle):  map hasher(blake2_128_concat) T::RoamingPacketBundleExternalDataStorageHash => Option<Vec<T::RoamingPacketBundleIndex>>;

        /// Get roaming packet_bundle session
        pub RoamingPacketBundleSession get(fn roaming_packet_bundle_session): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<T::RoamingSessionIndex>;

        /// Get roaming session's packet bundles
        pub RoamingSessionPacketBundles get(fn roaming_session_packet_bundles): map hasher(blake2_128_concat) T::RoamingSessionIndex => Option<Vec<T::RoamingPacketBundleIndex>>

        // /// Get roaming packet_bundle operator
        // pub RoamingPacketBundleOperator get(fn roaming_packet_bundle_operator): map hasher(blake2_128_concat) T::RoamingPacketBundleIndex => Option<T::RoamingOperatorIndex>;

        // /// Get roaming operator's packet bundles
        // pub RoamingOperatorPacketBundles get(fn roaming_operator_packet_bundles): map hasher(blake2_128_concat) T::RoamingOperatorIndex => Option<Vec<T::RoamingPacketBundleIndex>>
    }
}
