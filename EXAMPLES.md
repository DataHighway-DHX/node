...
Note: If you get error building like: `Cannot start service alice: OCI runtime create failed`, then try `docker-compose --file docker-compose-custom.yml --verbose down --build` or try running `./scripts/docker-clean.sh` (beware this deletes all Docker containers, images, and cache for all your projects, not just Datahighway), and then `rm -rf ./target/rls/debug` a few times until it no longer says `Directory not empty`
